...
long get_user_choice(int expected_inputs[], size_t len_expected_inputs);
long continue_with_selection();
double basic_calculator();
void number_analyzer();
...
int main(){
    ...
    while(user_wants_utility){
        choice = get_user_choice(menu_expected_inputs, len_menu_expected_inputs);
        
        if(choice == 1){
            while(choice == 1){
                float answer = basic_calculator();
                printf("\nAnswer (rounded to 4 decimal places): %.4f\n", answer);
                
                long user_wants_to_continue = continue_with_selection();
            ...
            while(choice == 3){
                float answer = temperature_converter();
...
long get_user_choice(int expected_inputs[], size_t len_expected_inputs){
    long value = 0;
    char user_choice[] = "";
    size_t user_choice_size = sizeof(user_choice) / sizeof(user_choice[0]);
...
long continue_with_selection(){
    ...
    long user_continue = get_user_choice(continue_inputs, len_continue_inputs);
...
double basic_calculator(){
    ...
    num_inputs = get_user_choice(calculator_expected_inputs, len_calculator_expected_inputs);
    ...
    if(num_inputs == 1){
        ...
        operation = get_user_choice(one_input_expected_operations, len_one_input_expected_operations);
        ...
        operation = get_user_choice(two_input_expected_operations, len_two_input_expected_operations);
...
void number_analyzer(){
    ...
    int selection = get_user_choice(analyzer_expected_inputs, len_analyzer_expected_inputs);
...
double temperature_converter(){
    ...
    current_temp_scale = get_user_choice(temperature_converter_inputs, len_temperature_converter_inputs);
    ...
    if(current_temp_scale == 1){
        ...
        desired_output_scale = get_user_choice(convert_from_fahrenheit_inputs, len_convert_from_fahrenheit_inputs);
        ...
        desired_output_scale = get_user_choice(convert_from_celsius_inputs, len_convert_from_celsius_inputs);
        ...
        desired_output_scale = get_user_choice(convert_from_kelvin_inputs, len_convert_from_kelvin_inputs);
